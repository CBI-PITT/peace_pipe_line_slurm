...
class remove_background(ImageOperation):
    def __init__(self, input, output, **kwargs):
        ...
        self.mask_folder = os.path.join(
            self.output_operation_folder,
            f'resolution_level_{self.resolution_level}',
            f'channel_{self.channel}',
            f"bg_mask{previous_operation}{self.z_suffix}"
        )
        self.prerequisites = kwargs.get('prerequisites', [])
        print("remove_background prerequisites", self.prerequisites)
    ...
    def run(self):
        print("Running remove_background")
        provenance_file_path = self.create_provenance()
        job_ids = self.run_remove_background()
    ...
    def run_remove_background(self):
        path_to_task = os.path.join(self.jobs_folder, f"remove_background_rl{self.resolution_level}_c{self.channel}.sh")
        main_script = os.path.abspath(__file__)
        slurm_script = os.path.join(os.path.dirname(main_script), "do_remove_background.py")
        with open(path_to_task, 'w') as f:
            f.write('#!/bin/bash\n')
            f.write('\n')
            f.write(f"#SBATCH -J {self.user}-remove_background")
            f.write('\n')
            f.write(f"#SBATCH -o {self.jobs_folder}/slurm_%j.out")
            f.write('\n')
            f.write('\n')
            f.write("source /h20/home/lab/miniconda3/bin/activate remove_bg_py311_cuda124")
            f.write('\n')
            ...
            f.write(' ')
            f.write(self.mask_folder if ' ' not in self.mask_folder else f'"{self.mask_folder}"')
            f.write(' ')
        ...
        return submit_slurm_indices(
            ...
            existing_outputs=existing_z_indices(self.save_folder) & existing_z_indices(self.mask_folder),
